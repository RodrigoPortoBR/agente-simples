...
async def lovable_webhook(request: Request):
    ...
    try:
        ...
        if "application/json" in content_type:
            ...
            data = dict(form_data)
            print(f"📨 Dados Form recebidos: {data}")
        
        # Extrair mensagem e session_id
        user_message = data.get('message', '')
        session_id = data.get('sessionId', f"lovable_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        ...
        error_session_id = locals().get('session_id', f"error_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
...
if __name__ == "__main__":
    ...
    print("🔗 Integração: Lovable webhook ativo")
